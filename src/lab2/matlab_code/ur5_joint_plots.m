...
act_dir  = 'act_2.8_mu_d0.8/'; % change with the activity name
file_name = 'data';
...
% measured data
pos_med= [  data.q1(t_start:t_step:t_end), ...
...
vel_med= [   data.dq1(t_start:t_step:t_end), ...
...
accel_med= [data.ddq1(t_start:t_step:t_end), ...
...
for i=1:6
    ...
    plot(time(t_start:t_step:t_end), pos_med(:, i), '--r', 'linewidth', 2), hold on, grid on, box on
...
for i=1:6
    ...
    plot(time(t_start:t_step:t_end), vel_med(:, i), '--r', 'linewidth', 2), hold on, grid on, box on
...
for i=1:6
    ...
    plot(time(t_start:t_step:t_end), accel_med(:, i), '--r', 'linewidth', 2), hold on, grid on, box on
    %title(plot_name, 'interpreter', 'latex')
    xlabel('time (s)', 'interpreter', 'latex')
    ylabel(plot_name, 'interpreter', 'latex')
    xticks(0:1:5)
